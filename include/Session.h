...
struct WeatherForecastSample {
    uint8 m_sessionType;  // 0 = unknown, 1 = P1, 2 = P2, 3 = P3, 4 = Short P, 5
                          // = Q1 6 = Q2, 7 = Q3, 8 = Short Q, 9 = OSQ, 10 = R,
                          // 11 = R2 12 = Time Trial
...
struct PacketSessionData {
    ...
    const char* GetSessionName() {
        char* ret = new char[strlen(Session_Type[this->m_sessionType]) +
                             strlen(Track[this->m_trackId]) + 2];
        strcpy(ret, Session_Type[this->m_sessionType]);
        strcat(ret, "_");
        strcat(ret, Track[this->m_trackId]);
        return ret;
    }
